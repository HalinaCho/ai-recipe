...
/** 요리 하나짜리 끼니. 집계 규칙은 요리 수와 무관해서 이걸로 충분하다. */
function slot(date, mealType, recipeName, missing) {
  return {
    ...
    isHoliday: false,
    holidayName: null,
    dishes: [dish(recipeName, missing)],
  };
}

function dish(recipeName, missing, role = "side") {
  return {
    id: `d-${recipeName}`,
    role,
...
test("한 끼니 안의 여러 요리가 모두 집계된다 (FR-13-08)", () => {
  // 국이 두부를 쓰고 반찬도 두부를 쓰면 "두부 2끼니"가 아니라 한 줄에 둘로
  // 묶여야 한다 — 장 볼 때 필요한 건 "두부를 사야 한다"는 사실이다.
  const list = buildShoppingList([
    {
      date: "2026-08-24",
      mealType: "dinner",
      isHoliday: false,
      holidayName: null,
      dishes: [dish("된장찌개", ["두부"], "soup"), dish("두부조림", ["두부", "대파"])],
    },
  ]);

  const tofu = list.find((i) => i.normalizedName === "두부");
  assert.equal(tofu.usedIn.length, 2);
  assert.deepEqual(
    tofu.usedIn.map((u) => u.role),
    ["soup", "side"],
  );
  assert.equal(list.filter((i) => i.normalizedName === "두부").length, 1);
});
