...
test("같은 주에 같은 레시피가 두 번 배치되지 않는다 (FR-13-02)", () => {
  ...
  const ids = placed.map((slot) => slot.recipe.id);
...
test("레시피가 칸 수보다 적으면 중복을 허용해서라도 칸을 채운다", () => {
  ...
  assert.ok(placed.every((slot) => slot.recipe !== undefined));
});
...
test("재고가 완전히 비어도 모든 칸이 채워진다 (FR-13-03)", () => {
  ...
  assert.ok(placed.every((slot) => slot.score.matchRate === 0));
});
...
test("같은 입력이면 언제나 같은 식단표가 나온다", () => {
  ...
  const first = placeWeek(input()).map((slot) => slot.recipe.id);
  const second = placeWeek(input()).map((slot) => slot.recipe.id);
...
test("고정된 칸은 재생성해도 그 레시피와 source를 유지한다", () => {
  ...
  const placed = placeWeek({
    ...
    purchaseShares: NO_HISTORY,
    locked: new Map([
      [slotKey(slots[0].date, slots[0].mealType), { recipeId: "r3", source: "manual" }],
    ]),
  });

  assert.equal(placed[0].recipe.id, "r3");
  assert.equal(placed[0].source, "manual");
  assert.equal(placed[1].source, "auto");
});
...
test("고정된 레시피가 사라졌으면 칸을 비우지 않고 자동 배치로 되돌린다", () => {
  ...
  const placed = placeWeek({
    ...
    purchaseShares: NO_HISTORY,
    locked: new Map([
      [slotKey(slots[0].date, slots[0].mealType), { recipeId: "지워진id", source: "manual" }],
    ]),
  });
