...
test("어댑터가 등록되지 않은 발신자의 메일을 주더라도 파서로 넘기지 않는다", async () => {
  const supabase = createFakeSupabase({
    mail_connection: [connectionRow("conn-1", "a@naver.com")],
  });

  // 어댑터의 필터가 잘못 고쳐진 상황을 흉내낸다: 사적인 메일이 섞여 나온다.
  const leaked = {
    providerMessageId: "leak-1",
    from: "친구 <friend@naver.com>",
    subject: "주말에 뭐해?",
    receivedAt: "2026-08-13T12:00:00.000Z",
    bodyText: "사적인 내용",
  };
  ...
  const result = await runHouseholdSync(supabase, HOUSEHOLD, {
    deps: {
      createAdapter: () => ({
        provider: "naver",
        async fetchOrderMails() {
          return [leaked, mail("ok-1", "[쿠팡] 주문 완료")];
        },
        async fetchMessageById() {
          return null;
        },
      ...
      parseMail: async (message) => {
        seenByParser.push(message.providerMessageId);
        return { purchasedAt: "2026-08-13", items: MILK, status: "success" };
      },
    },
  });
  ...
  assert.deepEqual(
    seenByParser,
    ["ok-1"],
    "등록 도메인 메일만 파서(=LLM)로 넘어가야 한다",
  );
  assert.equal(result.processedMailCount, 1);
  assert.equal(supabase.db.inventory_item.length, 1);
});

test("접미사만 같은 남의 도메인은 등록 도메인으로 인정하지 않는다", async () => {
  const supabase = createFakeSupabase({
    mail_connection: [connectionRow("conn-1", "a@naver.com")],
  });

  const lookalike = {
    providerMessageId: "evil-1",
    from: "sale@notcoupang.com",
    subject: "[쿠팡] 주문 완료",
    receivedAt: "2026-08-13T12:00:00.000Z",
    bodyText: "피싱",
  };
  ...
  assert.deepEqual(seenByParser, [], "notcoupang.com은 coupang.com이 아니다");
});
