...
// ---------------------------------------------------------------------------
// FR-09-03 — 레시피 탭 분류 필터
// ---------------------------------------------------------------------------

test("URL로 들어온 분류를 걸러서 받는다", () => {
  assert.deepEqual(parseCategories("반찬,국&찌개"), ["반찬", "국&찌개"]);
  assert.deepEqual(parseCategories(" 반찬 , 밥 "), ["반찬", "밥"]);
});

test("모르는 값은 무시한다", () => {
  // 필터가 조용히 0건을 내는 것보다 그 값을 없는 셈 치는 게 낫다.
  assert.deepEqual(parseCategories("반찬,없는분류"), ["반찬"]);
  assert.deepEqual(parseCategories("없는분류"), []);
  assert.deepEqual(parseCategories(null), []);
  assert.deepEqual(parseCategories(""), []);
});

test("중복은 접힌다", () => {
  assert.deepEqual(parseCategories("반찬,반찬,밥"), ["반찬", "밥"]);
});

test("필터 목록은 실제 분류값과 같아야 한다", () => {
  // 우리가 만든 이름으로 번역하면 DB 값과 갈라져 필터가 0건을 낸다.
  // 후식은 목록에 있어야 한다 — 식단표에서만 빼고 레시피 탭에서는 본다.
  assert.ok(RECIPE_CATEGORIES.includes("후식"));
  assert.ok(RECIPE_CATEGORIES.includes("국&찌개"));
  assert.equal(new Set(RECIPE_CATEGORIES).size, RECIPE_CATEGORIES.length);
});
