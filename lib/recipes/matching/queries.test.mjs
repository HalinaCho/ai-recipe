...
// ---------------------------------------------------------------------------
// FR-09-09 — 다양성 (같은 재료 반복 억제)
// ---------------------------------------------------------------------------

test("같은 재료 레시피가 도배하지 않고 다른 재료 레시피가 상위권에 섞인다", async () => {
  const tofuRecipes = Array.from({ length: 4 }, (_, i) =>
    recipeRows(`r-tofu-${i}`, `두부요리${i}`, ["두부"]),
  );
  const potatoRecipe = recipeRows("r-potato", "감자조림", ["감자"]);
  const supabase = createFakeSupabase({
    inventory_item: [
      inventoryRow("inv-tofu", "두부", "2026-08-10"),
      inventoryRow("inv-potato", "감자", "2026-08-11"),
    ],
    recipe: [...tofuRecipes, potatoRecipe].map((entry) => entry.recipe),
    recipe_ingredient: [...tofuRecipes, potatoRecipe].flatMap(
      (entry) => entry.ingredients,
    ),
  });

  const recipes = await buildRankedRecipeList(supabase, HOUSEHOLD, 5);
  const top3Names = recipes.slice(0, 3).map((item) => item.name);

  assert.ok(
    top3Names.includes("감자조림"),
    "재료를 다 가진 레시피가 5개나 있어도, 다른 재료 레시피가 상위 3자리 안에 들어야 한다",
  );
});

test("오늘의 추천도 같은 재료로만 채워지지 않는다", async () => {
  const tofuRecipes = Array.from({ length: 4 }, (_, i) =>
    recipeRows(`r-tofu-${i}`, `두부요리${i}`, ["두부"]),
  );
  const potatoRecipe = recipeRows("r-potato", "감자조림", ["감자"]);
  ...
  assert.ok(
    recipes.some((item) => item.name === "감자조림"),
    "오늘의 추천 3개가 전부 두부 요리로만 채워지면 안 된다",
  );
});
...
test("무드로 거르면 그 무드에 걸리는 레시피만 남는다", async () => {
  const spicy = recipeRows("r-spicy", "매운제육볶음", ["돼지고기"], ["고추장"]);
  const mild = recipeRows("r-mild", "계란찜", ["계란"]);
  const supabase = createFakeSupabase({
    inventory_item: [
      inventoryRow("inv-pork", "돼지고기", "2026-08-10"),
      inventoryRow("inv-egg", "계란", "2026-08-10"),
    ],
    recipe: [spicy.recipe, mild.recipe],
    recipe_ingredient: [...spicy.ingredients, ...mild.ingredients],
  });
...
test("좋아요는 그 레시피의 주재료를 선호(+1)로 모은다", async () => {
  // r-doenjang = 된장국(두부, 애호박).
  const supabase = preferenceSupabase({
    recipe_preference: [preferenceRow("p-1", "r-doenjang", "like")],
  });

  const { ingredientAffinity } = await loadPreferenceSignals(
    supabase,
    HOUSEHOLD,
  );

  assert.equal(ingredientAffinity.get("두부"), 1);
  assert.equal(ingredientAffinity.get("애호박"), 1);
});

test("싫어요는 기피(-1)로 모은다", async () => {
  const supabase = preferenceSupabase({
  ...
  assert.equal(ingredientAffinity.get("연어"), -1);
  assert.equal(ingredientAffinity.get("레몬"), -1);
});

test("보통은 어느 재료의 선호도에도 반영되지 않는다", async () => {
  const supabase = preferenceSupabase({
  ...
  assert.equal(ingredientAffinity.size, 0);
});

test("북마크와 요리 이력도 선호(+1) 신호로 합쳐진다", async () => {
  const supabase = preferenceSupabase({
  ...
  const { ingredientAffinity } = await loadPreferenceSignals(supabase, HOUSEHOLD);

  // r-egg = 계란말이(계란, 대파, 당근 — 대파는 조미료 화이트리스트라 주재료에서
  // 빠진다), r-doenjang = 된장국(두부, 애호박).
  for (const name of ["계란", "당근", "두부", "애호박"]) {
    assert.equal(ingredientAffinity.get(name), 1);
  }
});

test("신호가 하나도 없으면 빈 맵이다", async () => {
  const supabase = preferenceSupabase({});
...
test("다른 가구의 취향 신호는 안 섞인다", async () => {
  ...
  assert.ok(
    !ingredientAffinity.has("연어"),
    "다른 가구가 좋아요한 재료가 섞이면 안 된다",
  );
});

test("좋아요 5·싫어요 1처럼 양쪽에 걸쳐도 순 비율로 반영되고 0으로 상쇄되지 않는다", async () => {
  // 실사용에서 확인된 버그 재현: 두부를 쓰는 레시피 6개 중 5개는 좋아요,
  // 1개는 싫어요 — Set 기반이었을 때는 "있음/없음"만 보고 완전히 상쇄돼
  // ingredientAffinity에 두부가 아예 없는(또는 0인) 결과가 나왔다.
  const tofuRecipes = Array.from({ length: 6 }, (_, i) =>
    recipeRows(`r-tofu-${i}`, `두부요리${i}`, ["두부"]),
  );
  const supabase = createFakeSupabase({
    recipe: tofuRecipes.map((entry) => entry.recipe),
    recipe_ingredient: tofuRecipes.flatMap((entry) => entry.ingredients),
    recipe_preference: [
      preferenceRow("p-0", "r-tofu-0", "like"),
      preferenceRow("p-1", "r-tofu-1", "like"),
      preferenceRow("p-2", "r-tofu-2", "like"),
      preferenceRow("p-3", "r-tofu-3", "like"),
      preferenceRow("p-4", "r-tofu-4", "like"),
      preferenceRow("p-5", "r-tofu-5", "dislike"),
    ],
  });
  ...
  assert.equal(ingredientAffinity.get("두부"), (5 - 1) / (5 + 1));
});
