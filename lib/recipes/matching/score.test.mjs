...
test("소진임박 보너스가 실제로 순위를 바꾼다", () => {
  ...
  // 점수에 연결되어 있는지 확인한다.
  const flattened = rankRecipes([계란말이, 두부조림], ownedAll, expiring, NONE, NONE, {
...
test("취향 신호가 하나도 없으면 예전처럼 재고 중심으로만 채점한다", () => {
  ...
  // preferredNames·dislikedNames를 아예 안 넘긴 것과 빈 Set을 넘긴 것이
  // 같아야 한다 — "신호 없음"의 두 가지 표현이 갈리면 안 된다.
  const withEmptySets = scoreRecipe(
    계란말이,
    owned("계란", "당근"),
    NONE,
    NONE,
  ...
  assert.equal(withNoSignal.score, withEmptySets.score);
...
test("좋아하는 재료를 쓰면 점수가 올라간다", () => {
  ...
  const withPreference = scoreRecipe(
    ...
    ownedBoth,
    NONE,
    owned("돼지고기", "양파"), // 취향 퀴즈 좋아요 + 북마크 + 요리 이력에서 모인 재료
  );
...
test("싫어하는 재료를 쓰면 점수가 내려간다", () => {
  ...
  const withDislike = scoreRecipe(
    ...
    NONE,
    NONE,
    owned("가지"), // 취향 퀴즈에서 싫어요로 매긴 레시피의 재료
  );
...
test("싫어요가 몰려도 점수는 0 밑으로 내려가지 않는다", () => {
  ...
  const match = scoreRecipe(생선구이, NONE, NONE, NONE, owned("생선"));
...
test("취향은 레시피가 아니라 재료 단위로 일반화된다", () => {
  ...
  const 계란말이 = recipe("계란말이", main("계란", "당근"));
  const likedIngredients = owned("감자"); // 감자조림을 좋아해서 모인 재료
