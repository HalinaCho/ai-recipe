...
test("전부 제철이 아니면 계수가 0.3까지 떨어진다", () => {
...
test("재료가 많아도 감점이 희석되지 않는다", () => {
  // 실제 "감귤콩샐러드"가 이 경우다. 주재료 8개 중 감귤 하나만 제철이
  // 아닌데, 순수 비율(1/8)로 깎으면 계수가 0.91까지밖에 안 내려가 8월에도
  // 그대로 상위에 남는다. 감귤이 주인공인 요리에서 그건 틀린 결과다.
  const eight = ["감귤", "완두콩", "병아리콩", "치커리", "수수", "율무", "양파", "두부"];
  const factor = seasonPenaltyFactor(eight, ["감귤"], 8);

  assert.ok(
    factor < 0.65,
    `재료가 많다고 감점이 희석되면 안 된다 (계수 ${factor.toFixed(3)})`,
  );
  // 그래도 전부 제철 아닌 경우(0.3)보다는 덜 깎여야 한다.
  assert.ok(factor > 0.3);
});

test("제철 아닌 재료가 많을수록 더 깎인다", () => {
  const mains = ["감귤", "수박", "두부", "양파"];
  const one = seasonPenaltyFactor(mains, ["감귤"], 4);
  const two = seasonPenaltyFactor(mains, ["감귤", "수박"], 4);
  assert.ok(two < one, "제철 아닌 게 늘면 계수가 더 내려가야 한다");
});
